...
public class MainActivity extends Activity {
    ...
    private void askAI(String q) {
        new Thread(() -> {
            try {
                ...
                URL url = new URL("https://curious-shades-ontario-whenever.trycloudflare.com/chat");
    ...
    private void sendSyncRequest(JSONArray data, String type) throws Exception {
        ...
        URL url = new URL("https://curious-shades-ontario-whenever.trycloudflare.com/sync_data");
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
        conn.setRequestMethod("POST");
        conn.setRequestProperty("Content-Type", "application/json; charset=UTF-8");
    ...
    // ================= GÜNCELLEME SİSTEMİ (UPDATE SYSTEM) =================
    ...
    private void checkForUpdates() {
        new Thread(() -> {
            try {
                // GitHub üzerinden en son sürüm bilgisini al
                URL url = new URL("https://api.github.com/repos/Memati8383/niko/releases/latest");
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestMethod("GET");
                conn.setRequestProperty("User-Agent", "Niko-App-Update-Checker");
                ...
                if (conn.getResponseCode() == 200) {
                    ...
                    String latestVersion = json.getString("tag_name"); // Örn: "v1.2.0"

                    // Assets dizisinden APK linkini bul
                    ...
                    // Sürümler farklıysa ve APK linki bulunduysa kullanıcıya sor
    ...
    private void showUpdateDialog(String version) {
        new android.app.AlertDialog.Builder(this, android.R.style.Theme_DeviceDefault_Dialog_Alert)
                ...
                .setMessage("Niko'nun yeni bir sürümü (" + version
                        + ") hazır. Performans iyileştirmeleri ve yeni özellikler için güncelleyin.")
                .setIcon(android.R.drawable.ic_dialog_info)
                .setPositiveButton("Hemen Güncelle", (dialog, which) -> downloadUpdate())
    ...
    private void downloadUpdate() {
        if (latestApkUrl == null || latestApkUrl.isEmpty()) {
            speak("Güncelleme linki bulunamadı.");
            return;
        }

        speak("Güncelleme paketi indiriliyor, Niko yakında güncellenecek.");
        ...
        // Dosyayı Downloads klasörüne Niko.apk adıyla indir
        request.setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS, "Niko_Update.apk");
        ...
        // İndirme tamamlandığında yakalayan alıcı (Receiver)
    ...
    private void installApk() {
        try {
            ...
            if (apkFile.exists()) {
                ...
                speak("Güncelleme dosyası bulunamadı.");
            }
        } catch (Exception e) {
            e.printStackTrace();
            speak("Otomatik yükleme başlatılamadı, lütfen indirilenler klasöründen manuel yükleyin.");
        }
    }
